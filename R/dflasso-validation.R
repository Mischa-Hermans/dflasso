...
#'
#' @description
#' A checklist for deciding whether a fitted model is worth acting on. It is five
#' steps run at the console, each calling a verb that already exists, and adds no
#' new function or option. Run them on a held-out test set the model never saw,
#' prepared the same way as training.
#'
#' One caveat first. On the solver path [regret()] re-solves held-out instances
#' on real costs, so the comparison is measured rather than inferred. On the
#' supplied-regret path the [dfl_score()] ranking is a correlational signal: with
#' no held-out decisions there is nothing to back-test it against, so a high score
#' marks an association with decision failure and stops short of proving cause or
#' guaranteeing anything. The steps below are for the solver path.
#'
...
#' Whether the rescued features are plausible. The scores measure association and
...
#' ```
#'
#' A plausible result has rescued features a domain expert would recognise, with
#' `proxy_score` clearly off the floor. Weaker signs: rescues that are noise
#' columns with no explanation, or every score barely above the floor. Read the
#' y-axis as labelled. Tracking when the decision goes wrong reports an
#' association with decision failure and says nothing about cause.
#'
#' The confound to name here is collinearity. A feature can score high only
#' because it is correlated with a genuinely decisive feature; the proxy sees the
#' shared movement and cannot separate the two. A high score then marks a lead to
#' investigate, and does not show that the feature itself drives the decision.
#'
...
#' exact number on one.
#'
#' @section What to have in hand before acting:
#' Four things point the same way before a fit is worth acting on: held-out
#' [regret()] lower relative to the spread; the result keeping its sign across a
#' couple of seeds; rescued features that make sense to an expert; and a count of
#' scored instances that is not tiny. A training-data number, a single seed, or a
#' hair-thin gap inside the spread is too little to act on.
#'
#' Several outcomes point the other way. A held-out regret that did not drop is a
#' correct result the package is built to give; the prediction-focused model
#' stands, with the comparison learned. Where the in-fit preview looked good but
#' held-out did not, the held-out number is the one that counts. Where a single
#' seed looked good but the regret sign flipped, the gain did not hold up. A gain
#' on a handful of instances, or on one split, is a lead to confirm rather than a
#' settled result.
#'
